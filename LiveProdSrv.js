		...
		if(!err){
			ccgConnectOSC(cgosc, function(err, response){
				if(!err){ term(response); }
					...
					if(err == 'disconnected'){term("\n"+dateTimeNow()+"\t^#^y^B CASPAR CG ^ \tCasparCG Server OSC issue");}}
				});
			term(response);		}
		else{
			if(err == 'disconnected'){term("\n"+dateTimeNow()+"\t^#^y^B CASPAR CG ^ \tDisconnected from CasparCG Server");}}
		});
...
function uiActions(uiActionReceived){
	...
	if(uiActionReceived.action == 'CCGconnect'){
		term("\n"+dateTimeNow()+"\t^#^B^W CASPAR CG ^ \tCasparCG control on Server "+cgsrv+":"+cgamcp+" - osc :"+cgosc)
